class RolesController < ApplicationController
  before_action :set_role, only: %i[ show edit update destroy users]
  after_action :verify_authorized
  # GET /roles or /roles.json

  def users
      authorize @role
      @page_title = "Users with this role: #{@role.name}"
      @pagy, @users = pagy(@role.users.order(:last_name))
  ...
  def index
    authorize :role
    @page_title = t("activerecord.models.role.other")
